import { ServiceAsCancelled, ServiceAsDraft, ServiceAsPaid } from "../model/serviceSchema.js";
...
export const getCancelledBookings = async (req, res) => {
    try {
        const user = await ServiceAsCancelled.find({ "Customer_Details.Customer_Email": req.body.Email }, { "Order_Details": 1, "Draft": 1, "Payment_Details": 1 });
        if (user) {
            return res.send(user)
        }
        else {
...
export const cancelService = async (req, res) => {
    try {
        const paidBooking = await ServiceAsPaid.findOne({ "Customer_Details.Customer_Email": req.body.Email, _id: req.body._id });
        if(paidBooking){
            return res.status(300).json('already cancelled');
        }
        let cancelledService = new ServiceAsCancelled(req.body);
        await cancelledService.save();
        await ServiceAsPaid.deleteOne({ "Customer_Details.Customer_Email": req.body.Email, _id: req.body._id });
